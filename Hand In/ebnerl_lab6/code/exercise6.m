...
disp('Click some point correspondences that are nearest to the camera.');
[x1s, x2s] = getClickedPoints(imgRectL, imgRectR); 
% to get a good guess
close all;

maxDisp = max( abs( x1s(1,:)-x2s(1,:) ) )
threshMulti = 2.0;
boundary = ceil(threshMulti * maxDisp);

dispRange = -boundary:boundary;
